...
class DataValidation:
        ...
        try:
            ...
            for base_column in base_columns:
                if base_column not in current_columns:
                    logging.info(f'Column":[{base} is not avaialble]')
    ...
    def drop_missing_values_columns(self,df:pd.DataFrame,report_key_name:str)->Optional[pd.DataFrame]:
        """
        This function will drop columns which contains missing values more than secified threshold
        df:Accepts pandas dataframe
        threshold:Percentage criteria to drop columns
        ======================================================================
        returns pandas data frame if atleast single column is availabe
        """
        try:
            threshold=self.data_validation_config.missing_threshold
            null_report=df.isnull().sum()/df.shape[0]
            # Selecting column name which contains null
            logging.info(f"Selecting columns which contains null above to {threshold} ")
    ...
    def data_drift(self,base_df:pd.DataFrame,current_df:pd.DataFrame,report_key_name:str):
        try:
            ...
            for base_column in base_columns:
                ...
                # null  hypothesis is that both columns data drawin from same distribution
                logging.info(f'hypothesis {base_column}:{base_data.dtype},{current_data.dtype}')
                same_distribution=ks_2samp(base_data,current_data)

                if same_distribution.pvalue>0.05:
                    # we are accepting null hypthesis
    ...
    def initiate_data_validation(self)->artifact_entity.DataValidationArtifact:
        try:
            ...
            if train_df_columns_status:
                logging.info(f"As all columns are avaialbale in the train df hence detecting datadrift")
                self.data_drift(base_df=base_df, current_df=train_df,report_key_name='data_drift_within_train_dataset')
            if test_df_columns_status:
                logging.info(f"As all columns are avaialbale in the test df hence detecting datadrift")
